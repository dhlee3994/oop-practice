...
/**
 * 요구사항 간단한 사칙연산을 할 수 있다. 양수로만 계산할 수 있다. 0으로 나누는 경우 IllegalArgumentException 예외가 발생한다. MVC패턴 기반으로 구현한다.
 */
class CalculatorV1Test {
    ...
    void operate(int operand1, String operator, int operand2, int result) throws Exception {
        assertEquals(result,
            CalculatorV1.calculate(new PositiveNumber(operand1), operator, new PositiveNumber(operand2)));
    }
